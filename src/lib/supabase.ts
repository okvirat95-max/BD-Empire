...
export async function uploadMarketplaceAsset(asset: Omit<MarketplaceItem, 'reviews' | 'downloadsTrend' | 'rating' | 'downloads'>, creatorId: string) {
  ...
  while (attempts < 6) {
    ...
    // 2. Handle foreign key constraint violations (PostgreSQL code 23503)
    if (error.code === '23503' && error.message) {
      // Typically happens if creator_id points to profiles(id) but profiles row is missing or not synced yet.
