...
export async function getOrCreateProfile(supabaseUser: any, defaultUsername?: string): Promise<User> {
  ...
  if (error && error.code === 'PGRST116') {
    ...
    // Resilient profile table insert
    ...
    try {
      ...
      // Handle cases where some unexpected columns exist or are missing
      if (err && err.code === '42703') {
        const match = err.message.match(/column "([^"]+)"/);
        if (match && match[1]) {
          delete newProfile[match[1]];
          const { error: retryErr } = await supabase.from('profiles').insert([newProfile]);
          insertError = retryErr;
        }
      }
    } catch (e) {
      console.warn('Profiles table insert exception:', e);
    }
    ...
    // Explicitly sync separate table metrics
    try {
      await supabase.from('profile_xp').upsert([{ user_id: supabaseUser.id, xp: xpValue, level: levelValue, next_xp: nextXpValue }]);
    } catch (e) {
      console.warn('profile_xp upsert failed:', e);
    }

    try {
      await supabase.from('profile_currency').upsert([{ user_id: supabaseUser.id, tokens: tokensValue, diamonds: diamondsValue }]);
    } catch (e) {
      console.warn('profile_currency upsert failed:', e);
    }
...
export async function fetchMarketplaceItems(): Promise<MarketplaceItem[]> {
  ...
  for (const asset of assets) {
    ...
    let uploadType = asset.upload_type || 'file';
    let downloadUrl = asset.download_url || undefined;
    let fileName = asset.file_name || undefined;
    let detailedDescription = asset.detailed_description || '';
    let creatorUsername = asset.creator_username;
    let creatorAvatarBg = asset.creator_avatar_bg || 'bg-slate-800';
    let creatorAvatarEmoji = asset.creator_avatar_emoji || '🎮';
    let creatorIsVerified = asset.creator_is_verified || false;

    if (detailedDescription && detailedDescription.includes('<!--METADATA:')) {
      try {
        const match = detailedDescription.match(/<!--METADATA:({.*?})-->/s);
        if (match && match[1]) {
          const meta = JSON.parse(match[1]);
          if (meta.downloadUrl) downloadUrl = meta.downloadUrl;
          if (meta.uploadType) uploadType = meta.uploadType;
          if (meta.fileName) fileName = meta.fileName;
          if (meta.creatorUsername) creatorUsername = meta.creatorUsername;
          if (meta.creatorAvatarBg) creatorAvatarBg = meta.creatorAvatarBg;
          if (meta.creatorAvatarEmoji) creatorAvatarEmoji = meta.creatorAvatarEmoji;
          if (meta.creatorIsVerified !== undefined) creatorIsVerified = meta.creatorIsVerified;
          detailedDescription = detailedDescription.replace(/<!--METADATA:({.*?})-->/gs, '').trim();
        }
      } catch (e) {
        console.warn('Error parsing inline asset metadata:', e);
      }
    }
...
export async function uploadMarketplaceAsset(asset: Omit<MarketplaceItem, 'reviews' | 'downloadsTrend' | 'rating' | 'downloads'>, creatorId: string) {
  // 0. Pre-sync profile in the database to guarantee the foreign key constraints exist and match
  if (creatorId) {
    try {
      const targetUsername = asset.creator?.username || 'SandboxUser';
      
      // Attempt to upsert/insert the profile row with the exact expected username to satisfy foreign key constraints
      ...
      if (existingProfile) {
        profilePayload.username = targetUsername || existingProfile.username;
        profilePayload.rank = existingProfile.rank || 'USER';
        profilePayload.avatar_url = existingProfile.avatar_url || asset.creator?.avatarEmoji || '';
        profilePayload.email = existingProfile.email || '';
      } else {
        profilePayload.rank = 'USER';
        profilePayload.avatar_url = asset.creator?.avatarEmoji || '';
      }
      ...
      if (upsertErr) {
        console.warn('Profiles table pre-sync upsert error:', upsertErr);
      } else {
        console.log(`Profiles pre-sync completed successfully. Guaranteed creator username: ${targetUsername}`);
      }
    } catch (upsertErr) {
      console.warn('Profiles pre-sync exception:', upsertErr);
    }
  }

  // Format metadata for resilience against missing columns
  const metaObject = {
    downloadUrl: asset.downloadUrl,
    uploadType: asset.uploadType,
    fileName: asset.fileName,
    creatorUsername: asset.creator?.username,
    creatorAvatarBg: asset.creator?.avatarBg,
    creatorAvatarEmoji: asset.creator?.avatarEmoji,
    creatorIsVerified: asset.creator?.isVerified
  };
  const serializedMeta = `\n\n<!--METADATA:${JSON.stringify(metaObject)}-->`;
  const encodedDetailedDescription = (asset.detailedDescription || '') + serializedMeta;

  const payload: any = {
    id: asset.id,
    title: asset.title,
    description: asset.description,
    detailed_description: encodedDetailedDescription,
    category: asset.category,
    downloads: 0,
    ...
    upload_type: asset.uploadType || 'file',
    download_url: asset.downloadUrl || null,
  ...
  while (attempts < 6) {
    // Clone the payload at each attempt to ensure no reference/mutability issues in request builders
    ...
    lastError = error;
    console.warn(`Resilient insert attempt ${attempts + 1} failed:`, error);

    // 1. Handle missing/undefined columns error (PostgreSQL SQL engine 42703 OR PostgREST/Supabase schema cache error)
    if (error.message) {
      const matchPg = error.message.match(/column "([^"]+)"/);
      const matchPgrst = error.message.match(/Could not find the '([^']+)' column/);
      const matchGeneric = error.message.match(/column '([^']+)'/);
      
      const missingCol = (matchPg && matchPg[1]) || (matchPgrst && matchPgrst[1]) || (matchGeneric && matchGeneric[1]);
      if (missingCol) {
        console.log(`Pruning missing column from insert payload: ${missingCol}`);
    ...
    // 2. Handle foreign key constraint violations (PostgreSQL code 23503 or general message keyword matches)
    if (error.code === '23503' || (error.message && (error.message.includes('foreign key') || error.message.includes('fkey')))) {
      if (error.message && (error.message.includes('creator_username') || error.message.includes('creator_username_fkey')) && payload.creator_username) {
        console.log('Foreign key constraint failed on creator_username. Retrying insert without creator_username...');
        delete payload.creator_username;
        attempts++;
        continue;
      }
      if (payload.creator_id) {
        console.log('Foreign key constraint failed on creator_id. Retrying insert without creator_id...');
    ...
    // If it's another error we can't self-heal, break and throw
    break;
  }
